...
// 这段 Java 代码是一个示例程序，展示了如何使用 RocksDB Java API 来操作 RocksDB 数据库
public class RocksDBSample {
    // 静态代码块用于在类加载时加载 RocksDB 的本地库。这是使用 RocksDB Java API 之前必须的步骤
    static {
        RocksDB.loadLibrary();//这一步在干什么 有什么功能
    }

    public static void main(final String[] args) {
//        // 检查命令行参数，确保用户提供了数据库路径。
//        if (args.length < 1) {
//            System.out.println("usage: RocksDBSample db_path");
//            System.exit(-1);
//        }

        final String db_path = "./rocksdb-sample/";
        final String db_path_not_found = db_path + "_not_found";

        System.out.println("RocksDBSample");
        // 创建 Options 对象用于配置 RocksDB
        try (final Options options = new Options();
             final Filter bloomFilter = new BloomFilter(10);
             final ReadOptions readOptions = new ReadOptions()
                     .setFillCache(false);
             // 创建 Statistics 对象用于收集统计信息
            ...
            // 设置 RocksDB 的各种配置选项，例如创建缺失的数据库、设置统计信息、内存和压缩选项等
            try {
            ...
            try (final RocksDB db = RocksDB.open(options, db_path)) {
                ...
                // 使用 multiGetAsList 方法来同时获取多个键的值
